...
export interface AttackPayload {
  ...
  description: string;
  payload: string;
  category: 'reflected' | 'stored' | 'dom-based' | 'url-based' | 'event-handler';
...
export const XSS_ATTACK_PAYLOADS: AttackPayload[] = [
  ...
  // DOM Manipulation
  {
    ...
    payload: '<script>document.body.innerHTML="<img src=x onerror=alert(\'XSS\')>"</script>',
    category: 'dom-based',
    severity: 'high',
    expectedResult: 'escaped',
  },
...
export function runXSSAttackSimulation(): {
  ...
  console.log('%c=== XSS ATTACK SIMULATION & DRILL ===', 'font-size: 20px; font-weight: bold; color: #dc2626;');
  console.log('Simulating real-world XSS attacks against our defense mechanisms...\n');
  ...
  const categories = ['reflected', 'stored', 'dom-based', 'url-based', 'event-handler'] as const;
  
  categories.forEach(category => {
    const categoryPayloads = XSS_ATTACK_PAYLOADS.filter(p => p.category === category);
    if (categoryPayloads.length === 0) return;
    
    console.log(`%c${category.toUpperCase()} XSS ATTACKS`, 'font-size: 16px; font-weight: bold; color: #ea580c;');
    ...
    categoryPayloads.forEach((attack, index) => {
      ...
      console.log(`     Status: %c${testResult.toUpperCase()}`, `color: ${color}; font-weight: bold;`);
      console.log('');
    });
  });
  
  // Summary
  console.log('%c=== SIMULATION SUMMARY ===', 'font-size: 18px; font-weight: bold; color: #2563eb;');
...
if (typeof window !== 'undefined') {
  ...
  console.log('%cXSS Attack Simulation Loaded!', 'font-size: 14px; font-weight: bold; color: #dc2626;');
